...
const addSectionFlags = (text) => {
    return text.split('\n').map(line => {
        if (/^[^:\s]+:[^:\s]+$/.test(line)) {
            return line; // Keep lines like "BYBIT:BTCUSDT" as is
        }
        return `###${line.trim()}`; // Add "###" prefix to other lines
...
const processFile = data => {
    ...
    const flaggedSectionsText = addSectionFlags(sectionsText)

    console.dir(flaggedSectionsText)

    const pyld = flaggedSectionsText + `\n` + noDuplicatesData
